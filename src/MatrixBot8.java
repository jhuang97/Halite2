...
public class MatrixBot8 {
	
	private final String botName = "MatrixBot8"; // better fleeing/waiting for reinforcements
...
//    	Log.log(taskList.size() + " ");
    ...
    public static void main(final String[] args) {
    	MatrixBot8 myBot = new MatrixBot8();
        for (;;) {
    ...
    private void regroupAsNeeded() {
    	...
    	for (Integer i : thrustMoveIdx) {
    		...
    		double safetyDest = 0.7*(1.0*friendlyUndocked + 0.4*friendlyDocked)
    				...
    				- 0.95*enemyUndockedDest - 0.4*enemyDockedDest;
    		double ownCount = (shipList.get(i).getHealth() > 230) ? 0.96 : 0.6 + 0.3 * shipList.get(i).getHealth() / 255.0;
    		safety += ownCount;
    		safetyDest += ownCount;
    		
    		if (safety < 0 || safetyDest < -2) {
    			shipNeedRetreatIds.add(shipList.get(i).getId());
    			shipNeedRetreatIdx.add(i);
    		} else if (safetyDest < 0.5) {
    	...
    	for (Entry<Integer, Ship> ee : waitIdxAndEnemyShip.entrySet()) {
    		...
    		if (enemyShip == null) {
            	...
            	if (e != null) {
            		...
            		Position waitPoint = Navigation.waitPoint(gameMap, retreatTarget, enemyShip);
            		// go to wait point
