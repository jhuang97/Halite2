...
public class QueueBot11rb {
    ...
    public static void main(final String[] args) {
    	QueueBot11rb myBot = new QueueBot11rb();
        for (;;) {
        	myBot.performOneMove();
        }
    }
    
    private QueueBot11rb() {
        networking = new Networking();
        gameMap = networking.initialize("QueueBot11rb");
...
//        }
        ...
        for (ArrayList<Integer> cg : collisionGroups) {
        	if (cg.size() >= 2) {
            	...
            	Log.log(cg.toString());
            	revisedMoves = Navigation.reviseMovesCollision_v3(gameMap, cMoves, cShipList);
